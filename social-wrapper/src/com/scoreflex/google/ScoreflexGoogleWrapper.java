...
public class ScoreflexGoogleWrapper {
	...
	public static void logout(Activity activity) {
		...
		final PlusClient plusClient = newPlusClient(activity);
		Log.d("Scoreflex", plusClient.getAccountName());
		sCallbackStack.push(new SocialCallback() {
	...
	*/
	...
	protected static class PlusClientCallbackHandler implements PlusClient.ConnectionCallbacks, GooglePlayServicesClient.OnConnectionFailedListener {
		...
		public void onConnected(Bundle connectionHint) {
			...
			if (!sCallbackStack.empty()) {
				...
				new Thread(new Runnable() {
					...
					public void run() {
						try {
							final String accessToken = GoogleAuthUtil.getToken(
									mActivity, mPlusClient.getAccountName(),
									"oauth2:"+Scopes.PLUS_LOGIN +" "+GOOOGLE_SCOPE_EMAIL); // http://stackoverflow.com/questions/12689858/newly-released-authentication-with-google-play-services-problems-with-getting
		...
		public void onConnectionFailed(ConnectionResult result) {
			if (result.hasResolution()) {
				...
				try {
					result.startResolutionForResult(mActivity, REQUEST_CODE_START_RESOLUTION_FOR_RESULT);
				...
				if (!sCallbackStack.empty()) {
					...
					Log.w("Scoreflex", "Callback stack is empty, could not message error", e);
				}
			}
		}
	}
	protected static PlusClient newPlusClient(final Activity activity) {
		...
		PlusClientCallbackHandler handler = new PlusClientCallbackHandler(
				activity);
		PlusClient plusClient = new PlusClient.Builder(activity, handler,
				handler)
				.setVisibleActivities("http://schemas.google.com/AddActivity",
						"http://schemas.google.com/BuyActivity")
				.setScopes(Scopes.PLUS_LOGIN, GOOOGLE_SCOPE_EMAIL).build();
	...
	public static void login(Activity activity, SocialCallback callback)
		...
		try {
			...
			PlusClient plusClient = newPlusClient(activity);
			plusClient.connect();
	...
	public static void onActivityResult(Activity activity, int requestCode, int responseCode, Intent intent) {
		if (ScoreflexGoogleWrapper.REQUEST_CODE_START_RESOLUTION_FOR_RESULT == requestCode) {
			if (responseCode == Activity.RESULT_OK)
				newPlusClient(activity).connect();
			
			else {
				Log.i("Scoreflex", "Resolution for result with NOK response code");
				if (!sCallbackStack.empty()) {
					SocialCallback callback = sCallbackStack.pop();
					callback.call(null, new GoogleException(String.format("Resolution for result brought response code %d", responseCode)));
				} else {
